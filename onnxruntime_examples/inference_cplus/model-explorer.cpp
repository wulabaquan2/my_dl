...
int main(int argc, ORTCHAR_T* argv[]) {
 ...
 /* 
  ...
  Ort::Session session = Ort::Session(env, LR"(..\resnet18.onnx)", session_options);
  ...
  //input_tensors.emplace_back(vec_to_tensor<float>(input_tensor_values, input_shape));
   ...
   cv::Mat img = cv::imread(R"(..\images\cat.jpg)",
      cv::IMREAD_ANYCOLOR);
  img.convertTo(img, CV_32FC3, 1. / 255);
  cv::resize(img, img, cv::Size(224, 224)); //224*224*1*3
  //convert to 3*224*224
  cv::Mat img_transpose;
  img_transpose = img.reshape(1, 224 * 224).t();  // 224*224*1*3->(224*224)*3->3*(224*224)

   Ort::MemoryInfo mem_info =
       Ort::MemoryInfo::CreateCpu(OrtAllocatorType::OrtArenaAllocator, OrtMemType::OrtMemTypeDefault);
  input_tensors.push_back(Ort::Value::CreateTensor<float>(mem_info, (float*)img_transpose.data, img.total() * 3,
  ...
  try {
    ...
    //1*1000
    std::vector<float>floatarr_vec;
    float* floatarr = output_tensors.front().GetTensorMutableData<float>(); 
    floatarr_vec.assign(floatarr, floatarr + 1000);
    auto score=softmax(floatarr_vec);
    auto maxidx = std::distance(score.begin(), std::max_element(score.begin(), score.end()));
